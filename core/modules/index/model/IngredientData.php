...
class IngredientData {
	...
	public function del2(){
		$enlace = mysql_connect("localhost", "cinara", "carolina", "kevins");
		mysql_select_db('kevins');
	

		mysql_query("delete from ".self::$tablename." where id=$this->id");
...
// partiendo de que ya tenemos creado un objecto IngredientData previamente utilizamos el contexto
	public function update(){
		$sql = "update ".self::$tablename." set name=\"$this->name\",price_in=\"$this->price_in\",price_out=\"$this->price_out\",unit=\"$this->unit\",presentation=\"$this->presentation\",code=\"$this->code\" where id=$this->id";
		Executor::doit($sql);
	}
